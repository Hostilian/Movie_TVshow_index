(function() {
    console.clear();
    console.log("🚀 Starting V20 - CORRECT RA SYNTAX...");

    // PORTAL SYNTAX ANALYSIS:
    // ✓ MOVIE[title, year] - projection on single table
    // ✓ MOVIE(condition) - selection
    // ✓ TABLE × TABLE - cartesian product  
    // ✓ TABLE ∪ TABLE, TABLE ∩ TABLE, TABLE \ TABLE - set ops
    // ✓ TABLE *> TABLE - left outer join (FULL tables only)
    // ✓ TABLE < TABLE - natural join (FULL tables only)
    // ✗ (TABLE[cols] < TABLE) - parenthesis with projection FAILS
    // ✗ TABLE[cols] < TABLE[cols] - projection before join FAILS
    
    // THE SOLUTION: For joins, we must use FULL tables, no projection
    // Then RA and SQL must return the SAME columns (all of them)
    ...
    const queries = {
        ...
        "D4": {
            ...
            sql: "SELECT * FROM movie m NATURAL JOIN director d WHERE d.name = 'Christopher Nolan' ORDER BY m.movie_id;"
        },
        "D5": {
            cat: "D1",
        ...
        "D9": {
            ...
            ra: "(MOVIE < DIRECTOR)(imdb_rating > 8)",
