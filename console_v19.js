(function() {
    console.clear();
    console.log("🚀 Starting V19 - FIXING 'Unexpected end of input' errors...");

    // ANALYSIS OF ERRORS:
    // The portal says "RA query contains only declarations of relations"
    // This means "MOVIE < DIRECTOR" is being parsed as just table names
    // We need to use actual RA syntax with explicit operations
    
    // WORKING patterns from successful queries:
    // - MOVIE[title, year] ✓ (projection)
    // - MOVIE[movie_id] \ AWARD[movie_id] ✓ (set difference)
    // - DIRECTOR[name] ∪ ACTOR[name] ✓ (union)
    // - MOVIE[movie_id] ∩ AWARD[movie_id] ✓ (intersection)
    // - MOVIE(imdb_rating > 8) ✓ (selection)
    // - GENRE × COUNTRY ✓ (cartesian product)
    // - STUDIO *> COUNTRY ✓ (left outer join)
    
    // The < operator seems to need projection on at least one side
    // Let's try: TABLE[cols] < TABLE or TABLE < TABLE[cols]
    ...
    const queries = {
        ...
        "D4": {
            ...
            ra: "(MOVIE[movie_id, title, director_id] < DIRECTOR[director_id, name])(name = 'Christopher Nolan')[movie_id, title]",
        ...
        "D5": {
            ...
            ra: "(MOVIE_ACTOR[actor_id, movie_id] < MOVIE_GENRE[movie_id, genre_id])[actor_id, genre_id] ÷ GENRE[genre_id]",
        ...
        "D6": {
            ...
            ra: "((MOVIE_ACTOR[actor_id, movie_id] < MOVIE_GENRE[movie_id, genre_id])[actor_id, genre_id] ÷ GENRE[genre_id]) < ACTOR[actor_id, name]",
        ...
        "D7": {
            ...
            ra: "(MOVIE(year > 2015))[movie_id, title, year, director_id] < DIRECTOR[director_id, name]",
            sql: "SELECT DISTINCT m.movie_id, m.title, m.year, d.director_id, d.name FROM movie m JOIN director d ON m.director_id = d.director_id WHERE m.year > 2015 ORDER BY m.movie_id;"
        },
        "D8": {
            cat: "",
            natural: "List movie titles together with director names.",
            ra: "(MOVIE[title, director_id] < DIRECTOR[director_id, name])[title, name]",
        ...
        "D9": {
            ...
            ra: "((MOVIE(imdb_rating > 8))[title, director_id] < DIRECTOR[director_id, name])[title, name]",
        ...
        "D10": {
            ...
            ra: "(MOVIE[title, imdb_rating, director_id] < DIRECTOR[director_id, name])[name, title, imdb_rating]",
        ...
        "D11": {
            ...
            ra: "(MOVIE_GENRE[movie_id, genre_id] < GENRE[genre_id, genre_name])[genre_name, movie_id]",
        ...
        "D18": {
            ...
            ra: "(MOVIE[movie_id, title] < AWARD[movie_id])[movie_id]",
        ...
        "D19": {
            ...
            ra: "(MOVIE[movie_id, title] < AWARD[movie_id, award_name])[title, award_name]",
        ...
        "D20": {
            ...
            ra: "(MOVIE[movie_id, title] < AWARD[movie_id])[movie_id, title]",
        ...
        "D26": {
            ...
            ra: "STUDIO[studio_id, studio_name, country_id] *> COUNTRY[country_id, country_name]",
            sql: "SELECT s.studio_id, s.studio_name, s.country_id, c.country_id AS country_id_1, c.country_name FROM studio s LEFT JOIN country c ON s.country_id = c.country_id ORDER BY s.studio_id;"
        },
        "D27": {
            cat: "",
        ...
        "D29": {
            ...
            ra: "MOVIE[movie_id, title] *> AWARD[movie_id, award_name]",
            sql: "SELECT m.movie_id, m.title, a.movie_id AS movie_id_1, a.award_name FROM movie m LEFT JOIN award a ON m.movie_id = a.movie_id ORDER BY m.movie_id;"
        },
        "D30": {
            cat: "",
            natural: "List all directors together with their movies, including directors who have no films yet.",
            ra: "DIRECTOR[director_id, name] *> MOVIE[movie_id, title, director_id]",
            sql: "SELECT d.director_id, d.name, m.movie_id, m.title, m.director_id AS director_id_1 FROM director d LEFT JOIN movie m ON d.director_id = m.director_id ORDER BY d.director_id;"
        }
    };
