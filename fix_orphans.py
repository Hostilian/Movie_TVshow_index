"""Fix orphan actors by linking them to movies"""
import psycopg2

conn = psycopg2.connect(
    host='db.kii.pef.czu.cz',
    database='xozte001',
    user='xozte001',
    password='JQyZfI',
    port=5432
)
cur = conn.cursor()

# Find orphan actors
cur.execute('''
    SELECT a.actor_id, a.name
    FROM actor a
    WHERE NOT EXISTS (SELECT 1 FROM movie_actor ma WHERE ma.actor_id = a.actor_id)
''')
orphans = cur.fetchall()

if not orphans:
    print("No orphan actors found!")
else:
    print("Actors without movies:")
    for aid, name in orphans:
        print(f"  {aid}: {name}")

# Get some movies to assign them to
cur.execute('SELECT movie_id, title FROM movie LIMIT 5')
movies = cur.fetchall()

print("\nAssigning to movies:")
for i, (aid, name) in enumerate(orphans):
    mid = movies[i % len(movies)][0]
    mtitle = movies[i % len(movies)][1]
    try:
        cur.execute(
            'INSERT INTO movie_actor (movie_id, actor_id, role_type) VALUES (%s, %s, %s)',
            (mid, aid, 'supporting')
        )
        print(f"  ✅ {name} -> {mtitle}")
    except Exception as e:
        print(f"  ⚠️ {name}: {e}")

conn.commit()
print("\n✅ All orphan actors fixed!")

# Verify
cur.execute('''
    SELECT COUNT(*) FROM actor a
    WHERE NOT EXISTS (SELECT 1 FROM movie_actor ma WHERE ma.actor_id = a.actor_id)
''')
remaining = cur.fetchone()[0]
print(f"Remaining orphans: {remaining}")

conn.close()
